...
$amount = doubleval($data['total_amount']);
...
$sql="INSERT INTO `tbl_payments`(`service`, `amount`, `phone_number`, `duration`, `expires_at`,`status`,`order_id`,`trans_id`,`websessiontxnid`) VALUES ('$service','$amount','$phone','$duration','$expires_at','APP_RECEIVED','0','0','$registration_ticket')";
if(mysqli_query($db,$sql))
{
    /*$resp = createOrder($amount,$phone);
    if($resp['code'] == 200 && $resp['result']['data']['resultcode'] == "000") {
        $order_id= $resp['result']['order_id']; */
        $resp_pay = pay($amount,$phone); // push ussd
        if($resp_pay['success']) {
